...
def draw_waiting_screen(language_label: str):
    ...
    hint1 = _font_hint.render("다이얼을 돌려 언어를 선택하세요", True, COLOR_MUTED)
    _screen.blit(hint1, (20, 250))
    hint2 = _font_hint.render("버튼을 눌러 촬영을 시작하세요", True, COLOR_MUTED)
    _screen.blit(hint2, (20, 275))
...
def draw_result_screen(result: dict, scroll_offset: int = 0):
    ...
    # 하단 안내 문구 (스크롤과 무관하게 항상 고정 표시)
    hint_surf = _font_hint.render("다이얼: 스크롤 / 버튼: 재촬영", True, COLOR_MUTED)
    pygame.draw.rect(_screen, COLOR_BG, (0, SCREEN_HEIGHT - 22, SCREEN_WIDTH, 22))
    _screen.blit(hint_surf, (20, SCREEN_HEIGHT - 20))
